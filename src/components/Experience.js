    ...
    return (
        <section id="experience" className="py-20 bg-gray-900">
            <div className="container mx-auto">
                <h2 className="text-xl font-bold text-teal-400 mb-6 text-center">Experience</h2>
                <div className="ml-10 mr-10 grid grid-cols-1 md:grid-cols-2 gap-6">
                    {experiences.map((experience, index) => (
                        <div
                            key={index}
                            className="bg-gray-800 p-6 rounded shadow-lg hover:shadow-xl"
                        >
                            <h3 className="text-sm font-bold text-teal-400">{experience.company}</h3>
                            <p className="text-sm text-gray-300 mt-2">{experience.role}</p>
                            <p className="text-sm text-gray-500 mt-1">{experience.duration}</p>
                            <ul className="text-xs list-disc list-inside text-gray-300 mt-3 space-y-2">
